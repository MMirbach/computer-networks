...
def print_results():
    ...
    print(event.total_wait_time / event.num_of_vaccinated, # Tw - average waiting time
          (event.total_wait_time / event.num_of_vaccinated) + (event.total_service_time / event.num_of_vaccinated), # Ts - average service time
    event.num_of_vaccinated / time_limit) # lambda_A - average arrival time
